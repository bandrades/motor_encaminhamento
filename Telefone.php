...
class Telefone{
	...
	public function informaAreaTarifaria(){
		...
		$query = "SELECT areaTarifaria".
			" FROM motor_encaminhamento.tb_areaTarifaria".
			" WHERE ddd = ". $dddNumeroDiscado .
			" AND prefixo = ". $prefixoNumeroDiscado .
			" AND sufixoStart <= ". $sufixoNumeroDiscado .
			" AND sufixoEnd >= ". $sufixoNumeroDiscado;
	...
	public function informaDDD(){
		...
		if (strlen($this->numeroDiscado) == 8){
    		$dddNumeroDiscado = 34;
		} else if (strlen($this->numeroDiscado) == 11 || (strlen($this->numeroDiscado) == 12 && substr($this->numeroDiscado,3,1) == '9') ) {
   			$dddNumeroDiscado = substr($this->numeroDiscado,1,2);
		} else {
    		return null;
		}

		return $dddNumeroDiscado;
	}
